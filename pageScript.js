    ...
    if (event.data && event.data.type === '__fetchLocation') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          location_accurate = data?.data?.user_result_by_screen_name?.result?.about_profile?.location_accurate || false;
          source = data?.data?.user_result_by_screen_name?.result?.about_profile?.source || null;
